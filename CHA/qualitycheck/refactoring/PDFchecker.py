...
def main():
    ...
    if args.mode == "CHK" or args.mode == "PP":
        # CLI QC 책임자 확인
        clInterface.identify()
        # CLI 메뉴얼 보여주기
        clInterface.showManual()
        excelBase.doTask(args.mode, directory.figDir, clInterface.name)
    if args.mode == "EXP":
        # 출력모드! 기존에 있던 폴더 몽땅 날리기
        directory.refreshDir_all()
        excelBase.doExport(
            directory.includeDir, directory.excludeDir, directory.postponedDir
        )
